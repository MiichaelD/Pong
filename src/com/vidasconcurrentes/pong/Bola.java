...
public class Bola extends Rectangulo implements Movilidad, Runnable {

	public static final int UR = 1, UL = 2, DL = 3, DR = 4, VIBRAR_SENSILLO=1, VIBRAR_FINAL=2;
	private MediaPlayer mp;
	private PGView pgv;
	private Vibrator v = null;
	private int direccion,initX,initY,dormir=15, speedX,speedY;
	private boolean run;
	Raqueta raqL, raqR;
	Rect screen;
	
	public Bola(Point origen, int ancho, int alto,Raqueta rL,Raqueta rR, Rect scn, Context con, PGView pg) {
	...
	public int rebota(int x, int y, Rect screen,Rect raquetaIzda, Rect raquetaDcha) {
		...
		speedYchange(Math.abs(raqueta.centerY()-(origen.y+(alto/2))));
		}Log.v(PongActivity.TAG,"Bola - rebota: "+direccion);
		return 0;
	}
	
	public void speedYchange(int n){
		int h=alto/2;
		int distY=h-n;
		if(distY<(h/4)){
			speedY=5;
